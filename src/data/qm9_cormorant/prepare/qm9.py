...
def download_dataset_qm9(  # 用于下载并准备 QM9（也称为 GDB9）数据集。
    ...
    gdb9_url_data = "https://springernature.figshare.com/ndownloader/files/3195389"  # 完整xyz格式的数据集
    gdb9_tar_data = join(gdb9dir, "dsgdb9nsd.tar.bz2")
    # gdb9_tar_file = join(gdb9dir, 'dsgdb9nsd.tar.bz2')
    # gdb9_tar_data =
...
def gen_splits_gdb9(gdb9dir, cleanup=True):  # 自动生成 QM9（GDB9）数据集的分割。
    """
    ...
    gdb9_url_excluded = "https://springernature.figshare.com/ndownloader/files/3195404"  # uncharactered.txt
...
def get_thermo_dict(gdb9dir, cleanup=True):  # 其主要目的是从在线资源下载 QM9 数据集的分子热化学能量数据，并将其存储在一个字典中，以便后续使用
    """
    ...
    gdb9_url_thermo = "https://springernature.figshare.com/ndownloader/files/3195395"  # atomref.txt
...
def add_thermo_targets(data, therm_energy_dict):  # 目的是向 QM9 数据集的分子属性中添加热化学能量属性。
    """
...
def get_unique_charges(charges):  # 计算每个分子中每种电荷的数量
    """
