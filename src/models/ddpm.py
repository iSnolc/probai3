...
class DDPM(torch.nn.Module):
    ...
    def _q_mean(
        ...
        # print(sqrt_alpha_bars.shape)
        # print(sqrt_alpha_bars)
        # print(z.shape)
        # print(z)
    ...
    def losses(
        ...
        # print(h.shape[0]) # 暴力修改长度不可行
        # print(x.shape[1])
        # new_shape = torch.Size([h.shape[0], x.shape[1]])
        # x = torch.reshape(x, new_shape)
        # print(x)  # 有效17个，x是个张量tensor，想办法把其中的[ 0.0000,  0.0000,  0.0000]全都消除掉
        # print(h)  # 73个原子

        # tensor = x  # 清理x张量
        # # 找到非零向量的索引
        # nonzero_indices = torch.nonzero(tensor.sum(dim=1))
        # new_tensor = tensor[nonzero_indices.squeeze()]
        # x = new_tensor

        # print(x.shape)  # 116
        # print(h.shape)  # 68
        if h is not None:
            z = torch.cat([x, h], dim=-1)  # 拼接正确，排除错误来源
        else:
            z = x
